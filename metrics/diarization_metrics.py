...
class DiarizationMetrics(Metrics):
    ...
    def get_score(self, candidates, references, ids=None, lengths=None):
        ...
        # Speaker count MAE
        avg_speaker_count_absolute_error = sum(scores['spkcnterr'])/ len(scores['spkcnterr']) if scores['spkcnterr'] else 0
    ...
    def compute_record_level_scores(self, candidates: list, references: list):
        ...
        for i, (reference, candidate) in enumerate(tqdm(zip(references, candidates), desc="diarization_metrics", total=len(references))):
            ...
            try:
                ...
                raise ValueError("The ground truths are not labeled correctly. Reference speakers and reference transcripts are not aligned by turns.")
            try:
                assert len (cand_by_lines) == len(cand_speakers)
            except:
                raise ValueError("The generated outputs are not formatted correctly. Hypothesis speakers and hypothesis transcripts are not aligned by turns.")
            ...
            result = compute_wder(numeric_word_level_ref_speakers, numeric_word_level_cand_speakers, flattened_ref_transcripts, flattened_cand_transcripts, align, result)            
